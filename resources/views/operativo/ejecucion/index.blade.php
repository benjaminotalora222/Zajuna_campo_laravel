...
<div class="px-8 py-8 max-w-screen-xl mx-auto" style="color:#1c2b16;">
    ...
    {{-- FILTROS --}}
    <form method="GET" action="{{ route('operativo.ejecucion.index') }}"
        ...
        @if(request()->hasAny(['search','proyecto_id','prioridad']))
            <a href="{{ route('operativo.ejecucion.index') }}" class="px-5 py-2.5 rounded-xl font-bold text-sm border hover:bg-gray-50" style="border-color:#e7e0cc; color:#5a5a4f;">Limpiar</a>
        @endif
    </form>
...
const R = {
    store:   '{{ route("operativo.ejecucion.store") }}',
    base:    '/operativo/ejecucion/',
    csrf:    '{{ csrf_token() }}',
};
...
async function onDrop(event, nuevoEstado) {
    ...
    const res = await fetch(`/operativo/ejecucion/${draggingId}/estado`, {
        method: 'PATCH',
        headers: {
