...
<div class="px-8 py-8 max-w-screen-xl mx-auto" style="color:#1c2b16;">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
            ...
            <p class="text-sm mt-1" style="color:#5a5a4f;">Consulta stock y registra entradas/salidas.</p>
        </div>
    </div>

    <form method="GET" action="{{ route('operativo.inventario.index') }}"
        ...
        <div class="min-w-[140px]">
            <select name="stock" class="w-full px-4 py-2.5 rounded-xl border text-sm outline-none"
                    style="border-color:#e7e0cc; background:#fafafa;">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        @if($productos->isEmpty())
            <p class="px-6 py-16 text-sm text-center" style="color:#9a9a8a;">No hay productos en inventario.</p>
        @else
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @foreach($productos as $p)
                            ...
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-5 py-3.5 text-sm" style="color:#5a5a4f;">{{ $p->proveedor?->nombre ?? '—' }}</td>
                                ...
                                <td class="px-5 py-3.5">
                                    @if($loteProximo)
                                        <div class="text-sm">{{ $loteProximo->fecha_vencimiento?->locale('es')->isoFormat('D MMM YYYY') }}</div>
                                        <div class="text-xs" style="color:#9a9a8a;">{{ $loteProximo->cantidad_disponible }} {{ $p->unidad }} disponibles</div>
            ...
            @if($productos->hasPages())
                <div class="px-5 py-4 border-t border-gray-100">{{ $productos->links() }}</div>
            @endif
        @endif
    </div>
</div>
