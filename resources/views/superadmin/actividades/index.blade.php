...
<div class="px-8 py-8 max-w-screen-xl mx-auto" style="color:#1c2b16;">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-6">
        <form method="GET" action="{{ route('superadmin.actividades.index') }}" class="flex flex-wrap gap-3 items-end">
            ...
            @if(request()->hasAny(['search','proyecto_id','estado','fecha']))
                <a href="{{ route('superadmin.actividades.index') }}" class="px-5 py-2.5 rounded-xl font-bold text-sm border hover:bg-gray-50" style="border-color:#e7e0cc; color:#5a5a4f;">Limpiar</a>
            @endif
        </form>
    </div>
...
function abrirModalEditar(act) {
    ...
    document.getElementById('formActividad').action    = '/superadmin/actividades/' + act.id;
...
function confirmarEliminar(id) {
    document.getElementById('formEliminar').action = '/superadmin/actividades/' + id;
